...
// (5) resolve-throw on NOT-IN-MAP -> PdoAccessError. The runtime resolve path uses the one
// PdoAccessError tier for all three failures (not-in-map / width-mismatch / past-frame);
// PdoMappingError is RESERVED for the apply_pdo_map sub-protocol (#32) and must not re-broaden.
