...
struct ServoConfig {
    ...
    // The PP new-set-point ack timeout is an internal Cia402Policy constant (kHandshakeTimeoutCycles),
    ...
    // configured, so the policy's quick-stop SDO setup (0x605A assert, 0x6085 write/readback) is
