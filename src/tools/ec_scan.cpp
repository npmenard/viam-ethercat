...
void ec_sync(int64 reftime, int64 cycletime, int64* offsettime) {
    ...
    timeerror = -delta;
    integral += timeerror;
    *offsettime = (int64)(((float)timeerror * pgain) + ((float)integral * igain));
}
...
OSAL_THREAD_FUNC_RT ecatthread(void) {
    ...
    ht = (int)(ts.tv_nsec / 1000000) + 1; /* round to nearest ms */
            ...
            for (slaveix = 1; slaveix <= ctx.slavecount; slaveix++) {
                ...
                if ((slave->group == currentgroup) && (slave->state != EC_STATE_OPERATIONAL)) {
                    ...
                    if (slave->state == (EC_STATE_SAFE_OP + EC_STATE_ERROR)) {
                        ...
                        ecx_writestate(&ctx, (uint16)slaveix);
                        ...
                        if (ecx_reconfig_slave(&ctx, (uint16)slaveix, EC_TIMEOUTMON) >= EC_STATE_PRE_OP) {
                            slave->islost = FALSE;
                            printf("MESSAGE : slave %d reconfigured\n", slaveix);
                        }
                    } else if (!slave->islost) {
                        /* re-check state */
                        ecx_statecheck(&ctx, (uint16)slaveix, EC_STATE_OPERATIONAL, EC_TIMEOUTRET);
                ...
                if (slave->islost) {
                    if (slave->state <= EC_STATE_INIT) {
                        if (ecx_recover_slave(&ctx, (uint16)slaveix, EC_TIMEOUTMON)) {
                            slave->islost = FALSE;
        ...
        if (ctx.slavecount > 0) {
            ...
            for (int si = 1; si <= ctx.slavecount; si++) {
                ...
                if (slave->CoEdetails > 0) {
                    ecx_slavembxcyclic(&ctx, (uint16)si);
                    sdoslave = si;
            ...
            if (ctx.slavelist[0].state != EC_STATE_OPERATIONAL) {
                ...
                for (int i = 0; i < 5000; i++) {
                    printf("Processdata cycle %5d , Wck %3d, DCtime %12" PRId64 ", dt %8" PRId64 ", O:", cycle, wkc, ctx.DCtime, timeerror);

                    size = group->Obytes < 8u ? (int)group->Obytes : 8;
                    for (int j = 0; j < size; j++) {
                        printf(" %2.2x", *(ctx.slavelist[0].outputs + j));
                    }

                    printf(" I:");
                    size = group->Ibytes < 8u ? (int)group->Ibytes : 8;
                    for (int j = 0; j < size; j++) {
                    ...
                    if (sdoslave > 0) {
                        value = 0;
                        int sdo_wkc = ecx_SDOread(&ctx, (uint16)sdoslave, 0x1018, 0x02, FALSE, &size, &value, EC_TIMEOUTRXM);
                        (void)sdo_wkc;
                    }
...
int main(int argc, char* argv[]) {
    ...
    if (argc > 1) {
        /* create process data thread. SOEM's osal_thread_create*() take
         * func as `void*`, which C++ won't implicitly convert from a
         * function pointer, so cast explicitly. */
